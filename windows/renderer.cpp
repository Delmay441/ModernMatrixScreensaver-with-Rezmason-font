...
#include <cstdio>
#include <cmath>
#include "shaders_embed.h"   // generated by build.ps1: static const char kShaderHLSL[]
...
// Compile one entry point from the embedded HLSL source (self-contained .scr).
...
// True if the monitor hosting `hwnd` is currently in HDR mode (so an FP16 scRGB
// swapchain will actually show HDR highlights). On SDR displays we keep the
// proven B8G8R8A8 path.
...
bool Renderer::InitCore()
{
    ...
    ready_ = true;
    MMLog("Renderer ready: %ux%u  glyphs=%d (%dx%d)  textured=%d  headless=%d",
          width_, height_, atlas_.GlyphCount(), atlas_.Cols(), atlas_.Rows(),
          atlas_.HasTexture() ? 1 : 0, headless_ ? 1 : 0);
    return true;
}
...
bool Renderer::CreateDeviceAndSwapChain(HWND hwnd)
{
    ...
    if (headless_) return true;   // offscreen capture: no swapchain needed
    ...
    // Pick FP16 scRGB only on an HDR display; otherwise keep SDR B8G8R8A8.
    ...
    // scRGB linear color space for the FP16 HDR path.
    ...
    // We handle Alt+Enter ourselves; stop DXGI from intercepting it.
...
bool Renderer::CreateSceneTargets(UINT width, UINT height)
{
    ...
    bloomW_ = maxu(1, width / 2);
    bloomH_ = maxu(1, height / 2);
    if (!mk(bloomW_, bloomH_, bloomTex_[0], bloomRTV_[0], bloomSRV_[0])) return false;
    if (!mk(bloomW_, bloomH_, bloomTex_[1], bloomRTV_[1], bloomSRV_[1])) return false;
    return true;
}
...
bool Renderer::CreateShaders()
{
    ...
    if (!CompileShader("bloom_blur", "ps_5_0", b)) return false;
    MM_CHECK(device_->CreatePixelShader(b->GetBufferPointer(), b->GetBufferSize(), nullptr, &blurPS_));
...
{
    // Instance structured buffer (read by the vertex shader at t1).
    D3D11_BUFFER_DESC bd{};
    bd.ByteWidth = sizeof(MMGlyphInstance) * kMaxInstances;
    ...
    sd.BufferEx.NumElements = kMaxInstances;
    ...
    cd.ByteWidth = 16;   // float4 post params
...
void Renderer::Resize(UINT width, UINT height)
{
    ...
    if (FAILED(hr)) { MMLogHR("ResizeBuffers", hr); ready_ = false; return; }
    if (!CreateBackbufferRTV() || !CreateSceneTargets(width_, height_)) {
        MMLog("Resize: render-target recreation failed");
        ready_ = false;
        return;
    }
    ready_ = true;
...
void Renderer::UpdateUniforms()
{
    ...
    // Camera: a dead-level view by default; optional slow drift between framed
    // presets (PanController in Renderer.swift) when panning is enabled.
    ...
    if (settings_.panning) {
        ...
        const float seg = 9.0f;                          // seconds per move+dwell
        float cycle = panTime_ / seg;
        int i = (int)floorf(cycle) % 6; if (i < 0) i += 6;
        int j = (i + 1) % 6;
        float f = cycle - floorf(cycle);
        float t = f / 0.45f; if (t > 1.0f) t = 1.0f;     // move first 45%, then dwell
    ...
    XMMATRIX vp = XMMatrixMultiply(view, proj);   // row-vector: world * view * proj
    ...
    XMFLOAT3 e, r, cu;
    XMStoreFloat3(&e, eye); XMStoreFloat3(&r, right); XMStoreFloat3(&cu, camup);
    u.camX = e.x;  u.camY = e.y;  u.camZ = e.z;
    u.camRX = r.x; u.camRY = r.y; u.camRZ = r.z;
    u.camUX = cu.x; u.camUY = cu.y; u.camUZ = cu.z;
...
{
    // Stream this frame's visible glyphs into the instance buffer.
    D3D11_MAPPED_SUBRESOURCE ms;
    if (SUCCEEDED(ctx_->Map(instanceBuf_.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &ms))) {
        instanceCount_ = mm_sim_write_instances(sim_, (MMGlyphInstance*)ms.pData, kMaxInstances);
...
void Renderer::DrawPost()
{
    ...
    auto drawFS = [&](ID3D11RenderTargetView* rtv, UINT w, UINT h,
        ...
        ID3D11ShaderResourceView* nulls[2] = { nullptr, nullptr };
        ctx_->PSSetShaderResources(0, 2, nulls);            // avoid RTV/SRV hazard
    ...
    if (settings_.bloom) {
        setPost(0.72f, 0, 0, 0);                                   // knee
        drawFS(bloomRTV_[0].Get(), bloomW_, bloomH_, thresholdPS_.Get(), sceneSRV_.Get(), nullptr);
        const float tx = 1.0f / (float)bloomW_, ty = 1.0f / (float)bloomH_;
        for (int i = 0; i < 2; ++i) {
            setPost(tx, 0, 0, 0);
            drawFS(bloomRTV_[1].Get(), bloomW_, bloomH_, blurPS_.Get(), bloomSRV_[0].Get(), nullptr);
            setPost(0, ty, 0, 0);
            drawFS(bloomRTV_[0].Get(), bloomW_, bloomH_, blurPS_.Get(), bloomSRV_[1].Get(), nullptr);
        }
        setPost((float)settings_.bloomIntensity * 1.6f, 0, 0, 0); // intensity
        drawFS(backRTV_.Get(), width_, height_, compositePS_.Get(), sceneSRV_.Get(), bloomSRV_[0].Get());
    } else {
        setPost(0, 0, 0, 0);
...
void Renderer::DrawOverlay()
{
    ...
    // Wrap the current render target surface as a D2D bitmap and draw onto it.
    ComPtr<IDXGISurface> surface;
    if (headless_) { if (FAILED(finalTex_.As(&surface))) return; }
    else { if (!swap_ || FAILED(swap_->GetBuffer(0, IID_PPV_ARGS(&surface)))) return; }
    ...
    if (FAILED(d2dCtx_->CreateBitmapFromDxgiSurface(surface.Get(), &props, &target))) return;

    wchar_t buf[64];
    int n = swprintf(buf, 64, L"%.0f FPS", fps_);
    if (n <= 0) return;

    d2dCtx_->SetTarget(target.Get());
    d2dCtx_->BeginDraw();
    // A subtle shadow then the green text, for legibility over the rain.
...
static bool SavePNG(const wchar_t* path, UINT w, UINT h, const BYTE* bgra, UINT stride)
{
    ComPtr<IWICImagingFactory> factory;
    HRESULT hr = CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER,
                                  IID_PPV_ARGS(&factory));
    if (FAILED(hr)) return (MMLogHR("WIC factory", hr), false);

    ComPtr<IWICStream> stream;
    if (FAILED(hr = factory->CreateStream(&stream))) return (MMLogHR("CreateStream", hr), false);
    if (FAILED(hr = stream->InitializeFromFilename(path, GENERIC_WRITE)))
        return (MMLogHR("InitializeFromFilename", hr), false);

    ComPtr<IWICBitmapEncoder> enc;
    if (FAILED(hr = factory->CreateEncoder(GUID_ContainerFormatPng, nullptr, &enc)))
        return (MMLogHR("CreateEncoder", hr), false);
    ...
    if (FAILED(hr = frame->WritePixels(h, stride, stride * h, (BYTE*)bgra)))
        return (MMLogHR("WritePixels", hr), false);
    frame->Commit();
    enc->Commit();
    return true;
}
...
bool Renderer::SaveScreenshot(const wchar_t* path)
{
    ...
    bool ok = SavePNG(path, width_, height_, (const BYTE*)ms.pData, ms.RowPitch);
